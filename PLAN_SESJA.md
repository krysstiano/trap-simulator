...
## NPC W PARKU — zróżnicowanie + kolizja z płotem + ławka (NPC ma siedzieć)

> Cytat użytkownika: "Zróżnicuj npc w parku. Aktualni NPC w parku to jeszcze NPC 'starego typu' te przed wieloma zmianami. Wyglądają gorzej. Zrób żeby po parku chodził jakiś biznesman, jakaś starsza osoba, ktoś z psem, tak żeby było tam ciekawie i immerysjnie. Pamiętaj o kolizjach. Płot ma kolizje. NPC nie mogą chodzić/przechodzić przez płot. NPC w parku, mają chodzić tylko po parku. Popraw trochę wygląd ławki w parku i NPC przy ławce, bo aktualnie wygląda to bardziej jak biurko i jakby npc stał przy biurku. Powinno to wyglądać jakby npc siedział na ławce."

### Aktualny stan

**Park NPC** (linia 615-636):
```js
const startPos=[
  {wx:200,wy:950,type:'runner'},
  {wx:380,wy:1050,type:'dogwalker'},
  {wx:550,wy:1100,type:'gardener'},
  {wx:600,wy:950,type:'elderly'},
  {wx:250,wy:1080,type:'couple1'},
  {wx:260,wy:1080,type:'couple2'},
];
```

6 NPC starego typu — różnią się od miejskich NPC (24 sztuki, 6 typów: casual, runner, business, elderly, kid, dogwalker — bardziej zróżnicowane palety skóry/włosów/ubrań).

**Płot parku** (linia 5366-5375 — kolizje GRACZA):
- x:0-672, y:864-1248
- Bramka: c=13, r=21-22 (wy 1008-1104) — jedyne wejście
- Płot ma 4 segmenty (góra, dół, prawy górny, prawy dolny)

**Problem ławki** — wygląda jak biurko, NPC stoi obok zamiast siedzieć.

### 1. ZRÓŻNICOWANIE NPC W PARKU

**Cel:** Zastąpić 6 starych NPC nowymi, wykorzystując **system z miejskich NPC** (linia 652+ — `npcTypes`, `skinPalette`, `hairPalette`, `clothPalette`, `accentPalette`).

**Proponowany skład 6 NPC w parku** (mix typowych "park-friendly"):

| Typ | Liczba | Charakterystyka | Pozycja startowa |
|-----|--------|-----------------|------------------|
| 🏃 runner | 1 | Biegnie po pętli parku | wx:200, wy:950 |
| 👔 business | 1 | Biznesman z teczką, wolniejszy spacer (przerwa lunchowa) | wx:300, wy:1000 |
| 👵 elderly | 1 | Starsza osoba z chodzikiem, bardzo wolno | wx:500, wy:1100 |
| 🐕 dogwalker | 1 | Spacerowicz z psem na smyczy | wx:380, wy:1050 |
| 👨‍👩 couple | 2 | Para spacerująca razem (jak teraz, ale lepszy wygląd z palet) | wx:250, wy:1080 + wx:260, wy:1080 |

**Wykorzystanie systemu rendering:**
- Park NPC mają używać **tych samych palet** co miejskie NPC (`skinPalette`, `hairPalette`, `clothPalette`)
- Odrysowanie tym samym kodem co `drawCityPed` (jeśli istnieje) — zamiast osobnego `drawParkPed`
- Każdy NPC ma swój losowy: skin, hair, cloth, accent (z palet)

**Propozycja:**
- Usunąć typ `gardener` (nie pasuje do "życia w parku")
- Dodać `business` (przerwa lunchowa w parku) — buduje immersję
- Zachować `runner` (już dobrze pasuje)
- Zachować `elderly` (z chodzikiem, jak miejski)
- Zachować `dogwalker` (z psem na smyczy)
- Zachować parę (`couple1`, `couple2`) — ale używać nowych palet

### 2. ŚCISŁA KOLIZJA Z PŁOTEM (NPC chodzą TYLKO po parku)

**Aktualny stan:** Park NPC mają losowy ruch (`dx`, `dy` w linii 630), ALE NIE MA logiki kolizji z płotem — mogą "wyjść" przez płot.

**Wymagana logika** w `drawParkLife()` lub osobnej `updateParkPeds()`:

```js
// Park bounding (uwzględnia płot — NPC mają chodzić w środku!)
const PARK_BOUNDS = {
  minX: 96,    // wewnętrzna lewa krawędź (płot na x=0, marża 96px)
  maxX: 660,   // wewnętrzna prawa (płot na x=672, marża 12px)
  minY: 880,   // wewnętrzna góra (płot na y=864, marża 16px)
  maxY: 1230,  // wewnętrzna dół (płot na y=1248, marża 18px)
};

function updateParkPed(p){
  // Test ruchu z odbijaniem od płotu
  let nx = p.wx + p.dx;
  let ny = p.wy + p.dy;
  if(nx < PARK_BOUNDS.minX || nx > PARK_BOUNDS.maxX){ p.dx = -p.dx; nx = p.wx + p.dx; }
  if(ny < PARK_BOUNDS.minY || ny > PARK_BOUNDS.maxY){ p.dy = -p.dy; ny = p.wy + p.dy; }
  p.wx = nx;
  p.wy = ny;
}
```
...
**Kluczowe zasady:**
- ❌ NPC NIGDY nie wychodzi poza `PARK_BOUNDS`
- ❌ NPC NIGDY nie używa bramki (c=13, r=21-22) — to wejście dla GRACZA
- ✅ Marże uwzględniają wymiary samego NPC (ok. 16px szerokość ciała) + grubość płotu (FT=14)
- ✅ Odbicie kierunku przy próbie wyjścia (nie clamp — bo NPC byłby "przyklejony" do ściany)

**Kolizja z obiektami parku** (fontanna, breakdance, ławka, staw):
- NPC nie wchodzą NA fontannę/staw (bo to wygląda głupio)
- Sprawdzać kolizję z `park.objects` które mają `solid` lub specjalne flagi
- Podobnie jak gracz w linii 5390 (`walls.filter(o=>o.solid)`)

**Test:**
- Symulacja 60 sekund — żaden park NPC nie ma `wx<96 || wx>660 || wy<880 || wy>1230`
- Żaden park NPC nie pojawia się poza parkiem na ulicy

### 3. ŁAWKA W PARKU — wygląd + NPC SIEDZĄCY

**Aktualny problem:** ławka wygląda jak biurko (płaska, bez nóg/oparcia widocznych), NPC stoi obok zamiast siedzieć.

**Wymagania ławki:**
- **Realistyczny kształt ławki**:
  - Drewniane listwy poziome (siedzisko 3-4 listwy)
  - Pionowe oparcie (3-4 listwy lub kratka)
  - 2 metalowe nogi po bokach (kute żelazo, eleganckie)
  - Cień pod ławką
- **Wymiary**: szerokość ~70-90px, wysokość siedziska ~16px, oparcie +20px
- **Kolor**: brąz drewno (#7a4a18 lub ciemniejszy), nogi czarne/szare metalowe
- **Detal**: subtelny gradient/refleks na drewnie

**NPC siedzący — wymagania:**
- **Pozycja**: siedzi na środku ławki, plecy oparte o oparcie
- **Animacja**: **brak chód-animacji nóg** (siedzi nieruchomo)
- **Sylwetka**:
  - Tułów wyprostowany przy oparciu
  - **Nogi zgięte** (kolana wystają do przodu, stopy na ziemi przed ławką)
  - Ręce na kolanach lub luźno po bokach
- **Wysokość renderowania**: środek ciała przy poziomie siedziska ławki (ok. -8px od top ławki)
- Co jakiś czas (raz na 30s) NPC może wstać i przejść kawałek, potem znów usiąść — opcjonalne

**Funkcja `drawSittingNPC()`** (analogicznie do `drawDziadek` z chodzikiem):
```js
function drawSittingNPC(sx, sy, palette){
  // Tułów (siedzący — wyprostowany)
  X.fillStyle = palette.cloth;
  X.fillRect(sx-6, sy-22, 12, 18); // tułów
  // Głowa
  X.fillStyle = palette.skin;
  X.fillRect(sx-5, sy-32, 10, 10);
  // Włosy
  X.fillStyle = palette.hair;
  X.fillRect(sx-5, sy-32, 10, 4);
  // Nogi zgięte (kolana do przodu)
  X.fillStyle = palette.pants;
  X.fillRect(sx-5, sy-4, 4, 8); // lewa noga (kolano)
  X.fillRect(sx+1, sy-4, 4, 8); // prawa noga
  // Stopy na ziemi
  X.fillStyle = '#1a1a1a';
  X.fillRect(sx-6, sy+4, 5, 3); // lewa stopa (przed)
  X.fillRect(sx+1, sy+4, 5, 3); // prawa stopa
  // Ręce na kolanach
  X.fillStyle = palette.skin;
  X.fillRect(sx-7, sy-12, 3, 8); // lewa ręka
  X.fillRect(sx+4, sy-12, 3, 8); // prawa ręka
}
```

### 4. STRATEGIA "KTO SIEDZI NA ŁAWCE" — ✅ WYBRANE: OPCJA B (DYNAMICZNIE)

> Wybór użytkownika: "opcja B"

**Mechanika dynamiczna:**
- NPC sami podchodzą do ławki, siadają, wstają, odchodzą
- Różni NPC siedzą o różnych porach dnia
- Maksymalnie **1 NPC na ławce na raz**
- Czasem ławka jest pusta (między zmianami)

**Stan każdego park NPC:**
```js
// W parkPeds[]:
{
  ...,
  state: 'walking',      // 'walking' | 'goingToBench' | 'sitting' | 'leavingBench'
  sitTimer: 0,            // ile jeszcze siedzi (gdy state='sitting')
  benchCooldown: 0,       // czas do następnej możliwej próby (po wstaniu)
  _sittingOn: null,       // referencja do ławki na której siedzi
}
```
...
// Ławka w parku ma:
{
  ...
  id: 'park_bench',
  wx: ..., wy: ...,       // pozycja
  sitX: ..., sitY: ...,   // dokładny punkt gdzie NPC siada (środek siedziska)
  _occupiedBy: null,      // referencja do NPC siedzącego, lub null
}
```

**Cykl życia (state machine):**

1. **state='walking'** (domyślny) — NPC chodzi po parku z normalnym ruchem
   - Co tick: szansa ~0.1% na próbę pójścia do ławki (jeśli `benchCooldown===0` i ławka pusta)
   - Po decyzji → state='goingToBench', cel = sitX/sitY ławki

2. **state='goingToBench'** — NPC idzie w kierunku ławki
   - Wektor ruchu: `(sitX - wx, sitY - wy)` znormalizowany
   - Co tick sprawdzać dystans → jeśli `<5px` → state='sitting'
   - Jeśli ławka została zajęta przez kogoś innego po drodze → state='walking', `benchCooldown=300` (5s)

3. **state='sitting'** — NPC siedzi
   - `sitTimer` losowy 30-60s (np. `1800-3600` ticków przy 60fps)
   - `_occupiedBy = ten NPC`, ławka.`_occupiedBy = ten NPC`
   - Renderowanie: `drawSittingNPC()` (sekcja 4 powyżej)
   - Brak ruchu (`dx=0`, `dy=0`)
   - Co tick: `sitTimer--` → jeśli 0 → state='leavingBench'

4. **state='leavingBench'** — NPC wstaje i odchodzi
   - Losowy kierunek (dx, dy ≠ 0), `benchCooldown=600` (10s — żeby nie wrócił od razu)
   - `_occupiedBy = null`, ławka.`_occupiedBy = null`
   - Po 1 ticku → state='walking'

**Zasady systemu:**
- ✅ **Maks 1 NPC na ławce** (`ławka._occupiedBy` chroni przed kolizją)
- ✅ **Cooldown 10s po wstaniu** — żeby ten sam NPC nie zaraz wracał
- ✅ **Szansa próby 0.1%/tick** — daje średnio ~16s między próbami przy 6 NPC = nowy gość siada co ~16-30s
- ✅ **Płynne podejście** — NPC widać jak idzie do ławki, nie teleportuje się
- ✅ **Pusty stan OK** — czasem nikt nie siedzi (między zmianami) — to naturalne

**Edge cases:**
- 2 NPC dążą do ławki jednocześnie → sprawdzanie `_occupiedBy` PRZED siadaniem (pierwszy wygrywa, drugi wraca do walking)
- NPC dochodzi do ławki ale dystans tylko mały (np. odbicie od płotu) → snap do `sitX/sitY`
- Save/load podczas siedzenia → `state` i `sitTimer` w save, restoruje pozycję

**Renderowanie:**
- `state='sitting'` → użyj `drawSittingNPC()` (zgięte nogi, plecy oparte)
- `state='goingToBench'` lub `'leavingBench'` → standardowy walking sprite (animacja chodu)
- `state='walking'` → standardowy walking sprite

**Wartości do dostrojenia:**
- `sitTimer`: 30-60s (1800-3600 ticków @ 60fps)
- `benchCooldown` po wstaniu: 10s (600 ticków)
- Szansa próby: 0.1% per tick per NPC
- Próg dystansu siadania: 5px

### Test akceptacyjny

- **Audyt wizualny**: park NPC wyglądają tak samo dobrze jak miejscy (różne palety skóry/włosów/ubrań)
- **6 typów**: runner, business, elderly, dogwalker, couple×2 — wszyscy widoczni w parku
- **Kolizja z płotem**: 60-sekundowy test — żaden park NPC nie wychodzi poza `PARK_BOUNDS` (96-660, 880-1230)
- **Bramka**: park NPC NIE używają bramki — tylko gracz (test: NPC odbija się od płotu w okolicy bramki też)
- **NIE wchodzą na fontannę/breakdance/staw** — kolizja z obiektami parku
- **Ławka**: wygląda jak ławka (drewno + metalowe nogi + oparcie), NIE jak biurko
- **NPC na ławce**: wygląda jakby SIEDZIAŁ (zgięte nogi, plecy oparte o oparcie, ręce na kolanach), NIE stał obok
- **Dynamicznie**: różni NPC siedzą o różnych porach (jeśli opcja B)

---

## DRZEWKO UMIEJĘTNOŚCI — PEŁNA NAPRAWA (po feedbacku gracza po F3)

> Cytat użytkownika: "Jak gracz ulepsza coś to powinien widzieć aktualny % i następny %, powinno się to zmieniać i aktualizować. np. gracz nie mając dodanego żadnego punktu w kurier widzi 0% i informacje, ze następny punkt 20%, jak doda punkt to widzi, ze aktualnie ma 20% i następny punkt 40%. Pizza ma opisy związane z dostawami, a w pizzeri nie robi się żadnych dostaw tylko robi się pizze. Myjarz to dziwne słowo, zamień na detailer. Ewolucje nie działają. Zrobiłem ewolucje myjni w drzewku umiejętności, umyłem ponad 5 aut i nie było żadnego auta premium. Zrobiłem ewolucje kuriera i też nic to nie daje. Wydaje mi się że drzewko umiejętności jest kompletnie zepsute, nie daje żadnych efektów i ma dziwne opisy. Zmywak ma +1 naczynie, a naczyń jest tyle samo, nic w pracy się nie zmieniło. Poza tym dodawanie naczyń to słaby pomysł. Budowlaniec ma power up związane z utratą energii, to też nie ma żadnego sensu. Odporność ewolucja daje wytrzymały choroby i obniżenia, chorób nawet nie mamy w grze, a obniżenia to nawet nie wiadomo dla gracza o co z tym chodzi. Szansa na unik obrażeń też bez sensu. Hierarchia w drzewku umiejętności ma być taka punkty umiejętności > power up > ewolucja, ewolucja ma dawać najwięcej lub jakiś fajny unikalny efekt, musi być unikatowa. Power up powinno być czymś lepszym lub jakimś ciekawym rozwinięciem zwykłego rozwoju punktami, a zrobiłeś np. w charyzma tak, że gracz dostaje 30% za zwykłe punkty umiejętności, a za power up 6%, to nie ma żadnego sensu. Opis 'co 5 dni darmowy nieoczkiwany hit' zmień na 'Co 5 dni nieoczekiwany hit'. Ewolucja logika nie dała żadnej minigry w banku. Ewolucja skupienie rekordzista w kategorii nonsensu... Nie wiem jak w ogóle mają działać bonusy szybkość minigier, gracz nawet nie wie jakich minigier, co przez gre jest traktowane jako minigra. +5%/pkt mniej błędów w minigrach też jest napisane w sposób bardzo nieprzyjazny dla gracza. Gracz musi się zastanawiać o co w ogóle chodzi. Z hazardem nie mam pojęcia o co chodzi. Robisz drzewko umiejętności hazard jak nawet kasyna nie ma w grze. Ukryj to, ale kod jeżeli jest dobry możesz sobie zostawić, bo będziemy w dalszych etapach rozwoju gry dodawać kasyno. Flow też nie wiadomo o co chodzi np. z 'jeśli nie raper'. Wygrywanie wszystkich freestyle battles automatycznie też raczej nie ma sensu. Przy power up Ceny się rozjeżdżają. Zamiast wszystkie być ładnie pod sobą to pierwsza i trzecia są razem w jednej linii a druga jest wystrzelona obok. Ustaw to równo, tak żeby na siebie nie nachodziły. Umiejętności dieta i powolny apetyt wydają się być bardzo do siebie podobne, zróżnicuj je. Power up mają się nazywać inaczej, w sposób związany z daną umiejętnością. Nie np. Mindful I, Mindful II, Mindful III tylko każda z tych nazw ma być inna. Umiejętność bieg to bez sensu, w grze nawet nie ma funkcji sprintu. NAJWAŻNIEJSZE ŻEBY UMIEJĘTNOŚCI, POWER UP I EWOLUCJA FAKTYCZNIE DZIAŁAŁY W GRZE!"

### KLUCZOWA HIERARCHIA EFEKTÓW (zasada projektowa)

```
SKILL POINTS (max 3-5)  <  POWER-UPS (3 etapy)  <  EVOLUTION (1 unikat)
   bazowy efekt          rozwiniecie/lepsze         najmocniejszy LUB unikatowy
```

**ZASADA:** każdy kolejny etap musi być LEPSZY od poprzedniego. NIGDY tak że pkt umie daje 30% a power up 6% (jak teraz w charyzmie).

**Przykład prawidłowej hierarchii (charyzma):**
- Skill 3 pkt: +30% bonus z NPC (10%/pkt)
- Power-up 3/3: SKUMULOWANE +18% extra (np. 6%+12%+18%) → razem +48%
- Evolution: +60% TOTAL lub unique mechanic (np. NPC sami zaczepiają)

### ✅ Lista zmian — 4 GRUPY

#### GRUPA 1: NAPRAWY UI

1. **% wyświetlanie aktualny → następny** — przy każdym skill pokazać "Aktualnie: 0%, Następny: +20%". Po dodaniu pkt: "Aktualnie: 20%, Następny: +40%". Ma się aktualizować dynamicznie.
2. **CENY POWER-UPS rozjeżdżają się** (UI bug) — pierwsza i trzecia razem w linii, druga wystrzelona obok. Ustawić równo pod sobą.
3. **Tabs/header obcięty** (z poprzedniego raportu) — naprawić layout

#### GRUPA 2: NAZWY I OPISY

4. **Pizza opisy DOSTAW → ROBIENIE PIZZY** — pizzeria robi pizze, nie dostawy. Power-ups: szybkość gotowania, jakość ciasta, kombinacje
5. **Myjarz → Detailer**
6. **Power-ups nazwy unikalne per etap** — NIE "Mindful I/II/III" tylko np. "Cisza umysłu / Wewnętrzna harmonia / Oświecenie"
7. **Opis "Co 5 dni darmowy nieoczekiwany hit" → "Co 5 dni nieoczekiwany hit"** (usunąć "darmowy")
8. **Opis "+5%/pkt mniej błędów w minigrach"** — nieprzyjazny, przepisać konkretnie
9. **Opis "+5%/pkt szybkość minigier"** — niejasne, gracz nie wie co to minigra
10. **Flow opis "jeśli nie raper"** — niezrozumiałe, zmienić
11. **Bonusy NIE mówić "minigra"** ogólnie, tylko konkretnie czego dotyczy
12. **Dieta vs Apetyt zróżnicować** — teraz podobne, dać unikalne efekty

#### GRUPA 3: SKILLS DO PRZEPROJEKTOWANIA / USUNIĘCIA

13. **Bieg → USUNĄĆ lub przeprojektować** — sprintu nie ma w grze, więc bonus prędkości jest bez sensu (faktycznie przyjmie gracz ale za małe efekt do "sprintu" który nie istnieje). Decyzja: usunąć Bieg LUB zostawić jako "+prędkość ogólna" (już działa) ale z innym opisem
14. **Hazard → UKRYĆ** w UI (kod helper getHazardLuckBonus zostawić — kasyno będzie w przyszłości). Nie pokazywać w drzewku.
15. **Zmywak +1 naczynie → INNY power-up** — to słaby pomysł, plus i tak nie działa. Np. +x% szansa kombinacji, szybkość, multiplier earn
16. **Budowlaniec utrata energii → INNY power-up** — bez sensu. Np. większa wypłata, krótszy czas pracy
17. **Odporność evolution "wytrzymały - choroby"** → przeprojektować, w grze nie ma chorób. Np. "Niezniszczalny: -50% obrażenia z głodu/energii"
18. **Odporność power-up "szansa unik obrażeń"** → bez sensu, zamiast tego np. "natychmiastowa regeneracja przy spadku zdrowia"
19. **Logika "szybkość minigier"** → to placeholder, faktycznie zaimplementować coś sensownego LUB zmienić na inny efekt
20. **Skupienie "mniej błędów"** → j.w.
21. **Flow evolution "wygrywasz auto" → BEZ SENSU** (zabija gameplay), zmienić np. na "+50% scoore ze freestyle"

#### GRUPA 4: EVOLUTION FAKTYCZNIE DZIAŁA (gracz testował i NIE działa)

⚠️ **NAJWAŻNIEJSZE!** Wszystkie 16 evolutions są placeholderami. Gracz wykupuje za 150k zł + 1 pkt umie i NIC SIĘ NIE DZIEJE w grze.

**EVOLUTION DO IMPLEMENTACJI** (każda unique mechanic):

| Skill | Evolution name | EFEKT W KODZIE |
|-------|----------------|-----------------|
| pizza | "Pizzaiolo Master" | +50% wszystkich zarobków z pizzerii |
| supermarket | "Lojalność Klientów" | +500 zł bonus dziennie (raz/dzień) |
| carwash | "Premium Service" | Co 5 myć — premium auto +500 zł (TRZEBA dodać licznik aut) |
| construction | "Brygadzista" | Każda zlecenie 2× wypłata |
| kurier | "Multi-zlecenie" | Każda sesja kuriera daje 2× zarobki (proste) |
| zmywak | "Kuchnia Master" | +50% zarobki zmywaka |
| youtuber | "Algorithm Master" | +50% views z każdego nowego filmu |
| beatmaker | "Producent" | +50% sprzedaży bitów |
| gamer | "E-sport Pro" | Bonus 500 zł / dzień |
| influencer | "Brand Owner" | Pasywny dochód 300 zł / dzień |
| soundeng | "Studio Owner" | Pasywny dochód 400 zł / dzień |
| kondycja | "Pasywna regeneracja" | Co utracone 5 pkt energii → +1 (już zaprojektowane, do wpięcia) |
| apetyt | "Wege Guru" | Owoce/warzywa (jeśli sa w grze) 2× sytość; alternatywnie -50% spadku głodu |
| odpornosc | "Niezniszczalny" | -50% obrażeń z głodu/energii (kumulacja z bazowym -30%) |
| charyzma | "Magnetyzm" | NPC sami zaczepiają z bonusami (lub: +50% NPC zarobki) |
| czytanie | "Mędrzec" | +1 darmowy pkt umie co 3 lvl postaci |
| kreatywnosc | "Geniusz" | Co 5 dni "Co 5 dni nieoczekiwany hit" (FIX nazwa) — +500 zł, +50 fanów |
| logika | "Strateg" | (kasyno nie istnieje) zamiast — +25% wszystkie zarobki kalkulacyjne |
| skupienie | "Master Mind" | Bonus quality DO WSZYSTKICH minigier które mają quality |
| drip_master | "Trendsetter" | Wszystkie ubrania -20% ceny + bonus sławy |
| scena | "Showman" | +30% zarobki z koncertów |
| flow | "Battle King" | +50% score freestyle |
| foto | "Visual Artist" | Posty IG +30% engagement |
| sila | "Athletyk" | Bonus +10 do max formy (110 zamiast 100) |
| medytacja | "Wewnętrzny spokój" | Stres nie spada poniżej 50% (NIGDY) |
| dieta | "Nutricionist" | Cooking 2× lepsze efekty |

**Bieg, Hazard, Networking** — usuwać/ukrywać ze względu na niefunkcjonalność lub brak feature'ów.

### IMPLEMENTACJA — kolejność

1. **Faza A**: nazwy + opisy + ukrycie hazardu + dieta/apetyt różnicowanie (1 commit, czysto SKILL_DEF)
2. **Faza B**: hierarchia % (skill < power-up < evolution) — przejrzeć wszystkie 30 skills (1 commit, balans)
3. **Faza C**: UI fixes (% display + ceny równo + header) — 1 commit
4. **Faza D**: Power-ups EFEKTY w kodzie — wpięcie wszystkich 90 (3×30) tam gdzie się da (3-5 commits per kategoria)
5. **Faza E**: Evolution EFEKTY w kodzie — 16 unique mechanics (5-10 commits)

### Test akceptacyjny

Najwazniejsze: **gracz testuje każdy power-up + evolution i WIDZI EFEKT**:
- Wykup ewolucję myjni → następne myte auto = premium auto +500 zł
- Wykup ewolucję kuriera → następna sesja = 2× zarobki
- Wykup power-up X → opis się sprawdza w grze
- Każdy skill ma % aktualny + następny w UI

⚠️ **NIE oznaczaj sekcji DONE dopóki gracz nie potwierdzi że WIDZI faktyczne efekty**.

---

## "LOSUJ POSTAĆ" — przycisk nie działa w kreatorze postaci

> Cytat użytkownika (2026-05-10): "losuj postać przy tworzeniu postaci nie działa"

### Problem

W kreatorze postaci (modal "Nowa Gra" → tworzenie postaci) jest przycisk "Losuj postać" / "Random" / podobny — po kliknięciu nie losuje losowych atrybutów (skin, hair, cloth, pants, name, itd.).

### Hipotezy

- Funkcja `randomizeCharacter()` lub podobna nie istnieje / ma błąd w nazwie
- onclick handler odwołuje się do nieistniejącej funkcji (sprawdzić console.error)
- Funkcja istnieje ale nie aktualizuje UI (brakuje renderowania po randomize)
- Może `Math.random()` daje wartości spoza zakresu indeksów dostępnych skin/hair colors

### Plan

1. Znaleźć przycisk w HTML (Grep "Losuj" lub "random" w kreatorze)
2. Sprawdzić handler — czy funkcja istnieje, czy onclick działa
3. Sprawdzić console errors po klik
4. Jeśli brak funkcji — napisać nową: losuje skin, hair, cloth, pants, shoes, generuje losowe imię z listy
5. Jeśli funkcja jest ale buggy — naprawić

### Test akceptacyjny

- Klik "Losuj postać" → KAŻDY atrybut się zmienia (skin, hair, kolor koszulki, kolor spodni, imię)
- Postać w podglądzie aktualizuje się natychmiast
- Działa wielokrotnie (klikam 5× = 5 różnych postaci)

---

## SYGNALIZATORY — nakładające się słupy + biała linia przez crosswalk

> Cytat użytkownika (2026-05-10): "Sygnalizatory na skrzyżowaniach dalej wyglądają dziwnie, np ten obok fryzjera nachodzi na inny sygnalizator, przez przejście dla pieszych przechodzi dziwna jasnia linia."

### Problem 1: Nakładanie słupów obok fryzjera

Skrzyżowanie obok fryzjera — słup sygnalizatora nachodzi/koliduje z innym słupem (prawdopodobnie sygnalizator pieszych nakłada się na sygnalizator aut, lub dwa słupy stoją zbyt blisko).

Lokacja fryzjera: prawdopodobnie ~wx=350, wy=700 (dolny lewy obszar) → najbliższe skrzyżowanie to V1H1 (864, 480) lub V1H2 (864, 2016). Zlokalizować dokładnie i sprawdzić KTÓRY słup nachodzi na który.

### Problem 2: Dziwna jasna linia przez crosswalk

Na zebrze (crosswalk białe pasy) widać **jasną pionową/poziomą linię** która nie powinna tam być.

**Możliwe źródła:**
- Linia środkowa jezdni (białe przerywane / ciągłe pasy) renderuje się **na crosswalk** zamiast się przerywać przed nim
- Wysięgnik sygnalizatora w niektórych pozycjach przecina pasy crosswalk
- Cień słupa rzucany na crosswalk
- Białe pasy crosswalk się powtarzają (pasek width = 0 nie zostawia luki)

### Plan

1. Zlokalizować fryzjera w `index.html` (Grep "fryzjer" lub `wx:350` itp.)
2. Sprawdzić które skrzyżowanie jest "obok fryzjera" + screenshot tego konkretnego skrzyżowania
3. W `drawTrafficLights()` (linia ~21388) sprawdzić pozycje słupów aut (4) + słupów pieszych (4 rogi) — czy się nakładają
4. W rysowaniu jezdni szukać białych linii środkowych — czy się przerywa na crosswalk
5. Sprawdzić wysięgniki — czy `quadraticCurveTo` nie wpada w obszar crosswalk (wytyczne: wysięgnik łukowaty na wysokości stemTop-40 do stemTop-50, nie powinien wpadać w pas crosswalk który jest ±96 od środka)
6. Naprawić nakładanie słupów (przesunąć słupy peds o kilka px)
7. Naprawić linię na crosswalk (przerywać linie środkowe ulicy w obszarze crosswalk)

### Test akceptacyjny

- Screenshot skrzyżowania obok fryzjera: brak nakładania słupów
- Crosswalki BIAŁE PASY bez dodatkowych linii / cieni / wysięgników na nich
- Wszystkie 9 skrzyżowań sprawdzone że nie ma takiego problemu (regression)

---

## SYGNALIZATORY — DALEJ ZEPSUTE po pierwszej iteracji (audyt głębszy)

> Cytat użytkownika (2026-05-10, po commit v1.1.0): "1. jest OK, 2. Dalej zepsute. Brzydko wygląda jasna, cienka linia przecinająca przejście dla pieszych. Jeden z sygnalizatorów na skrzyżowaniu 'zlewa się' z przejściem dla pieszych. Inny sygnalizator nachodzi na śmietnik, te stare sygnalizatory wystają na ulice. Skrzyżowania po prostu dalej wyglądają źle, napraw te rzeczy i popracuj nad wyglądem skrzyżowań."

### Konkretne problemy do naprawy

1. **Cienka jasna linia DALEJ przecina crosswalk** — moje hBlocked/vBlocked rozszerzenie do 70px nie wystarczyło LUB problem jest gdzie indziej (np. tile 'roadm' z żółtą linią rysowany jako kafelek pod jezdnią, albo inny rysunek)

2. **Sygnalizator "zlewa się" z crosswalk** — słup pieszych jest TAK BLISKO crosswalk że pasy zebra wizualnie nakładają się na słup (mimo że ja przesunąłem ±160). Może słup stoi NA pasie zebra.

3. **Sygnalizator nachodzi na śmietnik** — gdzieś słup peds (po przesunięciu ±160) wpada w pozycję śmietnika. Trzeba zlokalizować śmietniki + sprawdzić kolizje.

4. **"Stare sygnalizatory wystają na ulice"** — może są LATARNIE (drawStreetLamps) lub inne obiekty starego rendera z jeszcze poprzedniej iteracji. Audit czy są jakieś OLD signal obiekty.

5. **Ogólnie skrzyżowania źle wyglądają** — popracować nad wyglądem ogólnym

### Plan napraw

1. **Audyt 9 skrzyżowań ze szczegółowymi zoomami** (różne kąty, różne rogi)
2. **Zlokalizować śmietniki** (Grep "smietnik" / "kosz" / "trash") + ich pozycje
3. **Zlokalizować "stare sygnalizatory"** — czy są resztki kodu ze starej wersji
4. **Naprawić linię na crosswalk PONOWNIE** — zamiast 70px być może 100px+ blokady, lub usunąć całkowicie środkową linię w zakresie ±150 od skrzyżowań
5. **Słupy peds nie powinny stać NA pasach zebra** — sprawdzić pozycje vs zakresy crosswalk
6. **Słupy peds nie powinny kolidować ze śmietnikami / latarniami** — przesunąć lub usunąć kolidujące obiekty

### Test akceptacyjny

- Wszystkie 9 skrzyżowań ze screenami: brak białej linii na crosswalk
- Brak słupa stojącego na pasach zebra
- Brak słupa kolidującego ze śmietnikiem / latarnią
- Wszystko czytelne, schludnie ułożone

---

## SYGNALIZATORY — REGRESJA po redesign v1.1.1

> Cytat użytkownika (2026-05-10, po commit v1.1.1): "Teraz są zepsute jeszcze bardziej, wyglądają dziwnie i brzydko. W dodatku jeszcze jakieś białe linie. Te stare sygnalizatory (dla pieszych) stoją na ulicy."

### Problem

Mój redesign v1.1.1 (sygnalizatory aut nad właściwymi pasami + zmniejszone słupy peds + nowe wysięgniki) pogorszył wygląd zamiast poprawić.

Konkretne problemy:
1. Wyglądają dziwnie i brzydko (subiektywnie — coś z proporcjami / pozycjami)
2. Nadal białe linie
3. **Stare sygnalizatory dla pieszych STOJĄ NA ULICY** — czyli słupki peds są poza chodnikiem, na asfalcie

### Plan

1. **REVERT** zmian sygnalizatorów aut z v1.1.1 (były lepiej przed)
2. Zachować TYLKO: zmniejszone słupy peds + naprawa białej linii (vBlocked/hBlocked)
3. Naprawa pozycji słupów peds — UPEWNIĆ SIĘ że są na chodniku, nie na asfalcie:
   - Chodniki: V zachód 672-768, V wschód 960-1056, H N 288-384, H S 576-672
   - Słup peds powinien być NA chodniku, najlepiej tuż przy crosswalk
4. Naprawa białej linii — możliwe że to NIE drawVDash/drawHDash, ale coś innego (cienia, wysięgnik łukowy, inny obiekt)

---

## REGRESJA: nowa postać chodzi WOLNIEJ niż kiedykolwiek

> Cytat użytkownika (2026-05-10, po commit v1.1.2): "Znalazłem bug! Zacząłem nową grę i postać chodzi wolniej niż kiedykolwiek!"

### Problem

Nowa postać (forma=0, kondycja skill=0) chodzi POWOLI — wolniej niż kiedykolwiek. Regresja w jakimś commit z dzisiejszej sesji.

### Hipotezy

1. **Diagonal speed fix (1/√2)** — naprawione wcześniej, ale może zła normalizacja powoduje też redukcję w kierunkach prostych
2. **Forma/kondycja bonus znak odwrócony** — może bonus jest zamiast +X% to -X%
3. **Skill kondycja efekt** — może zmieniony i daje minus zamiast plus
4. **P.spd default** — może obniżony w jakiejś zmianie
5. **Strój sportowy** — może tylko z PRO daje normalną prędkość, bez stroju jest minus
6. **Mróz** — kurtka zimowa redukuje utratę zdrowia, ale może też redukuje prędkość?

### Plan

1. Grep `P.spd`, `playerSpeed`, `getKondycja`, `getBiegSpd`
2. Sprawdzić commit history dziś: które zmiany dotykały prędkość
3. Test w grze: zmierzyć faktyczną prędkość bazową
4. Naprawić źródło

---

## DŹWIĘK KROKÓW — duplikacja (drugie kroki w tle)

> Cytat użytkownika (2026-05-10, po commit v1.1.3): "Z dźwiękiem kroków też coś się stało. Teraz jak postać chodzi to poza naszym normalnym dźwiękiem kroków słychać jeszcze jakieś inne w tle."

### Problem

Gdy gracz idzie, słychać DWA rodzaje dźwięków kroków:
- Normalny (kroki postaci)
- Dodatkowy w tle (nieznany, regresja)

### Hipotezy

1. **Dwie funkcje grają kroki** — jedna stara (pre-fix), druga nowa
2. **Kroki NPC** — funkcja grająca kroki dla NPC w pobliżu została dopisana lub odpalana dla gracza
3. **Powiązane z F4.3 lub F5.4** — może diagonal fix lub outfit overlay przypadkiem dodaje dźwięk
4. **Audio loop** — jakaś pętla audio nie jest zatrzymywana

### Plan

1. Grep `playStep`, `playFootstep`, `step`, audio refs
2. Sprawdzić ostatnie commity czy nie wprowadziły dodatkowego dźwięku
3. Odsłuchać w grze (dbg z audio capture? albo sprawdzić tylko console.log)

---

## SKRZYŻOWANIA — ŁAD I ESTETYKA (3 iteracja, blokujące)

> Cytat użytkownika (2026-05-10): "Pracuj dalej nad skrzyżowaniami i sygnalizacją świetlną, bo blokuje to nas strasznie i nie możemy przez to pójść do przodu. Ja bym chciał po prostu żeby w kwestiach wizualnych był zachowany ład. Dalej mamy jakąś jasną, cienką linie przecinającą pasy od przejścia dla pieszych, mamy sygnalizacje zlewającą się z innym sygnalizatorem, sygnalizator nachodzący na śmietnik itp. Zrób żeby to wyglądało ładnie, schludnie, czysto i estetycznie."

### Konkretne problemy DALEJ aktualne

1. **Cienka jasna linia przez pasy crosswalk** — mimo blokady ±100px wciąż jest. Szukać POZA drawVDash/drawHDash. Może to:
   - Linia środkowa jezdni żółta (tile 'roadm' jeśli używana)
   - Cień słupa
   - Wysięgnik łukowy (subtelny ale jasny)
   - Linia kafelka chodnikowego (nowo dodana w v1.1.2!)
   - Stripe na kafelku jezdni

2. **Sygnalizacja "zlewająca się" z innym sygnalizatorem** — słup peds + słup aut wiszący w tym samym rogu. Jeden zakrywa drugi.

3. **Sygnalizator nachodzi na śmietnik** — są śmietniki w mieście (apt_trash, park_trash) blisko słupów peds.

### Kierunek napraw

A. Zlokalizować JEDNOZNACZNIE źródło białej linii (super-zoom + analiza pixeli)
B. Przesunąć słupy peds tak by NIE kolidowały ze słupami aut
C. Sprawdzić wszystkie obiekty (śmietniki, ławki, latarnie) blisko skrzyżowań i przesunąć słupy peds dalej
D. Generalny polish — proporcje, kolory, czytelność

### Test akceptacyjny

- 9 screenshotów (każde skrzyżowanie) — wszystko schludne
- BRAK białych linii / cieni / wysięgników na crosswalk
- Słupy peds odsunięte od śmietników i innych obiektów
- Słupy peds NIE nakładają się na słupy aut

---

## SKRZYŻOWANIA + NPC CROSSWALK — DALEJ ZEPSUTE (sesja 2026-05-10 końcówka)

> Cytat użytkownika (2026-05-10 koniec): "Skrzyżowania i sygnalizatory świetlne dalej zepsute (nienaprawione). NPC nie przechodzą po przejściach dla pieszych tylko obok przejść dla pieszych. Źle jest wytyczona ich trasa."

### Problem 1: Skrzyżowania DALEJ wyglądają źle (po 5 iteracjach)

Mimo: rozszerzonej blokady pasów (±100→±130), wykluczenia hydrantów/koszy z ±170 wokół skrzyżowań, zmniejszenia słupków peds — DALEJ ZEPSUTE wg user.

Konkretne źródło bug-a niezidentyfikowane (wszystkie testy pixel-scan pokazują 0 białych pixeli na crosswalkach po blokadzie). Może problem jest na innej rozdzielczości (memory: 2560×1440 bug) lub innym viewporcie.

### Problem 2: NPC trasa źle wytyczona

NPC chodzą OBOK przejść dla pieszych, nie NA pasach zebra.

Mój zakres `_crossXranges` w F6 E2 jest ZA SZEROKI:
- V1: 718-1010 — ten zakres OBEJMUJE CAŁĄ JEZDNIĘ V1 (768-960), nie tylko crosswalki!
- Faktyczne crosswalki: V1 W (718-768) i V1 E (960-1010)
- NPC może zacząć przechodzić STOJĄC NA ŚRODKU JEZDNI (wx=864) — wygląda jakby szedł "obok" pasów zebra

### Plan napraw

1. **Skrzyżowania:** poprosić user o screenshot z zaznaczonymi pixelami (bo pixel-scan automatyczny nie znajduje problemu)
2. **NPC crosswalk:** zwęzić `_crossXranges` do FAKTYCZNYCH pasów zebra:
   - Stary: `[[718,1010],[2158,2450],[3406,3698]]`
   - Nowy: `[[718,768],[960,1010],[2158,2208],[2400,2450],[3406,3456],[3648,3698]]` (6 zakresów po 50px każdy = pasy zebra W i E na 3 jezdniach)
3. **Animacja NPC:** facing pionowy podczas crossingu (P.dir 0/2)
4. **Path-finding:** opcjonalnie NPC może najpierw iść poziomo do najbliższego crosswalk, potem przechodzić

### Test akceptacyjny

- NPC przechodzą TYLKO po pasach zebra (wizualnie widać że są na białych paskach)
- NPC NIE przechodzą przez środek jezdni
- 30s obserwacji skrzyżowania pokazuje co najmniej 1-2 NPC na pasach zebra

---

## 2026-05-13 — LISTA UŻYTKOWNIKA (po kasynie v1.8.17/18)

**Kontekst:** User dał komplet bug-reportów po sesji z kasynem v1.8.17/18. Pkt 112-113 ZROBIONE w tej sesji (v1.8.18). Pozostałe odłożone do kolejnych implementacji.

> **Pełen cytat user (2026-05-13):** "Ustaw darmowy spin 15% na nic, 26% na 20 zł, 26% na 50 zł, 21% na 200 zł i 12% na 500 zł. Kasyno dalej może gracz bugować. Może np. kręcić ruletką, wychodzić, wchodzić ponownie i przyspieszac (omijac limit animacji) na slotach to samo. NPC dalej zbugowani [...]"

---

## 112. ✅ DONE v1.8.18 — Darmowy spin progi

> "Ustaw darmowy spin 15% na nic, 26% na 20 zł, 26% na 50 zł, 21% na 200 zł i 12% na 500 zł"

Zaimplementowane w `casinoDailySpin` linia 7680. Progi w kodzie ZGODNE 1:1 z cytatem.

---

## 113. ✅ DONE v1.8.18 — Kasyno anti-spam cooldown

> "Kasyno dalej może gracz bugować. Może np. kręcić ruletką, wychodzić, wchodzić ponownie i przyspieszac (omijac limit animacji) na slotach to samo"

Dodany `G._slotsCooldownUntil` (2.4s + 150ms) i `G._rouletteCooldownUntil` (3.5s + 150ms). Po wyjściu/wejściu blokada nowego spinu aż minie pełen czas animacji. Łączny rate = pełna animacja niezależnie od exit/re-entry.

---

## 114. NPC kręcą głową/oczami zamiast chodzić

> "NPC dalej zbugowani, teraz zamiast chodzić w miejscu, stoją i kręcą głową/oczami na boki bardzo szybko."

**Diagnoza:** v1.8.16 dodało `anim++` tylko gdy NPC ruszył się >0.15px. Ale nie ruszają się wcale → anim się nie inkrementuje → ale facing (kierunek twarzy/oczu) zmienia się szybko niezależnie. Prawdopodobnie bug w wyborze kierunku (zmiana co frame) zamiast trzymania jednego kierunku.

**Naprawa:** sprawdzić logikę `npc.dir`/`npc.facing` i upewnić się że zmienia się tylko gdy NPC FAKTYCZNIE zmienia kierunek ruchu, nie co frame. Plus jeśli NPC stoi (dx==0,dy==0) → facing stabilny.

---

## 115. NPC w dolnych przejściach — chodzą OBOK pasów

> "W dolne przejścia dla pieszych NPC dalej nie trafiają i nie przechodzą PO pasach tylko OBOK pasów."

Powiązane z #111 (skrzyżowania crosswalk) — ale konkretnie DOLNE przejścia. Sprawdzić `_crossXranges` dla dolnej jezdni, zwęzić do faktycznych pasów (jak w naprawie #111).

---

## 116. Gra daje fanów/subskrybcji za nic

> "Gra daje fanów za nic, zacząłem dopiero co grę i mam 67 fanów, a nie zdążyłem zrobić wiele, kilku zdobyłem grając (resztę dostałem za nic). Nie wrzuciłem jeszcze żadnego filmu a mam 43 subskrybcji, to bez sensu."

**Diagnoza:** Sprawdzić `G.fans` i `G.subscribers` startowe wartości w `newGame()` / `startGame()`. Powinno być 0 lub bardzo niski seed.

**Naprawa:** start: fans=0, subscribers=0. Audyt automatycznych przyrostów (passive income) — czy nie ma "darmowych" tickerów co dzień.

---

## 117. Streaming — osiągnięcie 7 dni za 1 stream

> "Odpaliłem stream raz, a dostałem osiągnięcie za streamowanie 7 dni pod rząd."

**Diagnoza:** logika `G.streamDays` / `G.streamStreak` jest błędna — inkrementuje przy każdym stream zamiast unikalnych dni.

**Naprawa:** sprawdzić warunek osiągnięcia, dodać kontrolę unique-day (G.lastStreamDay !== G.day przed inkrementem).

---

## 118. Streaming — spam "zakończ stream" mnoży pieniądze

> "Streamowanie można bugować, gracz może spamować zakończ stream i mnożyć pieniądze."

**Diagnoza:** brak guard'a `if(streaming) return;` w endStream() — można wywołać wielokrotnie i każdorazowo doliczać.

**Naprawa:** `if(!G._streamActive) return;` na początku endStream. Po zakończeniu `G._streamActive=false`.

---

## 119. YouTube — rozbudować, ma wyglądać jak prawdziwy

> "Youtube powinien być bardziej rozbudowany i bardziej wyglądać jak youtube faktycznie."

**Zakres:** UX overhaul telefon → YouTube tab. Sidebar (Subskrypcje/Trendy/Biblioteka), thumbnails z fake views/likes, komentarze pod filmem, monetyzacja per-video. Coś w stylu kasyna interior — wizualny redesign.

---

## 120. Spanie/odpoczywanie — za długo, przyspieszyć

> "Spanie/odpoczywanie musi zdecydowanie szybciej regenerować energie, bo aktualnie to zdecydowanie za długo."

**Naprawa:** boost regen rate w `restLoop()` / `sleepLoop()`. Sprawdzić current values i zwiększyć ~2-3×. Memory note: nie złamać balansu (energia/dzień).

---

## 121. Kebab budka — możliwość zakupu, pasywny dochód

> "Rozbuduj budkę z kebabem. Zrób żeby gracz miał możliwość zakupu budki z kebabem, po zakupie ma działać jak pasywne źródło dochodu."

**Zakres:**
- E na budce → opcja "Kup budkę" za X zł (np. 5000-10000)
- Po zakupie → automat ticker `G.kebabBudka.dailyIncome`
- Wyświetlanie statystyki w telefonie/bank: "Budka kebab: +200 zł/dzień"
- Opcjonalnie: można sprzedać / upgrade

---

## 122. Cykl dnia — brak wygładzenia o 06:00

> "Dalej nie ma wygładzenia między porami dnia, np. jak wbija 06:00 to robi się od razu jasno."

**Diagnoza:** v1.8.15 dodało SMOOTH interpolację (8 keyframes), ale wg user dalej widać skok o 6:00. Trzeba zweryfikować czy keyframe na 6:00 ma za ostry przeskok od 5:30.

**Naprawa:** dodać pośrednie keyframes 5:00/5:30/6:00/6:30/7:00 z stopniowymi wartościami brightness/shadow.

---

## 123. Blackjack split As — dziwne zachowanie

> "W kasynie przy split as dziwnie się zachowuje w blackjack."

**Naprawa:** sprawdzić handler `_bjSplit()` dla par AA. Standardowe casino rules: split AA daje TYLKO 1 dodatkową kartę na każdą rękę (brak hit po). Możliwe że nasz kod pozwala na multi-hit lub source kart nie randomizuje.

---

## 124. Brak cinematic przy zakupie studia

> "Nie ma cinematic przy zakupie studia."

**Naprawa:** dodać cinematic overlay przy zakupie studia (podobnie jak penthouse VIP / pierwsza wizyta w kasynie). Studio kupienie = duży milestone.

---

## 125. Interior studia — jak kasyno (pełen rebuild)

> "Interior kasyna zrobiłeś dosyć ładny, zainspiruj się nim i zrób podobny interior studia. Ma studio wyglądać na prawdę jak studio muzyczne."

**Zakres:** studio room rebuild:
- Konsola miksująca z faderami
- Kabina nagraniowa za szybą + mikrofon na statywie
- Komputer z DAW (Pro Tools / Ableton mock UI)
- Słuchawki, monitory, akustyczne panele na ścianach
- Płyty platynowe na ścianie (achievement-based)
- Bar z drinkami / kanapa dla gości
- Oświetlenie nastrojowe

---

## 126. Studio — mikrofon kupiony, nie można nagrać wokalu

> "Mam mikrofon kupiony, a nie mogę nagrac wokalu w studio."

**Diagnoza:** sprawdzić warunek odblokowujący tryb "Nagraj wokal" w studio. Powinno być `G.inventory.includes('microphone')` lub `G.hasMic`.

---

## 127. Studio pipeline — bit → miks → mastering

> "Po zrobieniu bitu powinno przenosić do miksu i później do masteringu."

**Naprawa:** workflow studio:
1. Bit (beatmaker) → save jako `G.draftBeats[]`
2. Po bicie → opcja "Przejdź do miksu"
3. Mix minigra (faders/EQ) → save z `mixed=true`
4. Po miksie → "Mastering" (final polish)
5. Po mastering → "Wydaj utwór" → spotify/YT

---

## 128. Studio — pełna analiza mechaniki

> "Sprawdz całą mechanikę studia, przeanalizuj ją dobrze pod kątem sensu i logiki i napraw to."

Audyt: kariera Realizatora vs Beatmaker, wymagania, progres, sense. Połączyć z #125-127.

---

## 129. Bity bez rozwiniętej kariery

> "Bity można robić np. bez w ogóle rozwiniętej ścieżki kariery."

**Diagnoza:** warunek dostępu do mikser-bit nie checkuje `G.skills.beatmaker > 0` lub podobnie.

**Naprawa:** dodać wymóg minimalny skill / odblokowanie progresywne.

---

## 130. Sklep muzyczny — brak "posiadasz" + duplikaty zakupów

> "W sklepie muzycznym kupując niektóre rzeczy nie wyświetla się nic w ogóle, ani posiadasz ani nic kompletnie innego. Niektóre te same rzeczy można kupować w kółko pomimo, że się je już ma."

**Naprawa:**
- Po zakupie → tag "✓ Posiadasz" wyświetlany na itemie
- Blokada przycisku Kup jeśli już posiadane (lub upgrade tier)
- Audyt wszystkich itemów w `MUSIC_SHOP`

---

## 131. Postać — wygląd ogólny, dredy z jasnymi końcówkami, avatar, fryzjer

> "Popracuj nad ogólnym wyglądem naszej postaci [...] dredy wyglądały lepiej (mają wciąż mieć jasne końcówki), zrób żeby postać na avatarze wyglądała lepiej, na podglądzie u fryzjera lepiej."

**Zakres duży:** drawPlayer rebuild + avatar drawer + fryzjer preview drawer. Spójność wszystkich miejsc gdzie postać jest rysowana.

---

## 132. Koncerty — spełnione wymagania, nie można rezerwować

> "Z koncertami jest coś nie tak. Mam spełnione wymagania, a nie mogę zarezerwować żadnego koncertu."

**Diagnoza:** sprawdzić warunki `bookConcert()` — czy fame/skills check jest sensowny, czy nie wymaga jakiegoś ukrytego flag'a.

---

## 133. Penthouse noc — animacja okien (odważniki)

> "Popracuj nad wyglądem penthouse w nocy, chodzi o tą animację którą widać w oknach w nocy, wygląda to jak skaczące odważniki przemieszczające się na bok"

v1.7.x dało obrotowy reflektor RGB + fajerwerki — wg user dalej brzydkie. Trzeba zmienić motion: zamiast "skaczących odważników" sylwetki gości (taneczne), świece, glow. Bardziej life-style niż "podskakujące prostokąty".

---

## 134. HUD — losowa ikonka obok pieniędzy

> "Czasami obok pieniędzy na HUD pojawia się jakaś ikonka i liczba, nie wiadomo co to jest."

**Diagnoza:** prawdopodobnie `showGain()` floaty layered nad HUD lub jakiś debug indicator. Sprawdzić `updateHUD()` i element przy money node.

---

## 135. Parking — auta jak limuzyny + wjazd + dziura

> "Parking wygląda dziwnie. Auta znajdujące się na parkingu wyglądają wszystkie jak limuzyny (każde auto jest nienaturalnie długie). Wjazd miałeś zrobić na parking, wjazd miał zastąpić część chodnika [...] niektóre linie parkingowe są ucięte, parking dalej jest nierówny. Parking powinien być od chodnika do chodnika i wypełniać całą tą przestrzeń a po prawej stronie parkingu jest dziura."

**Zakres:**
1. Auta parking — różne typy (sedan, hatchback, SUV) z różnymi długościami, nie wszystkie 123px
2. Wjazd — wyciąć fragment chodnika (drawSidewalk omija parking entry), zastąpić tarmac
3. Linie parkingowe — dopasować do faktycznej szerokości, nie urywać
4. Wypełnić całą przestrzeń chodnik-do-chodnika, usunąć dziurę po prawej

---

## 136. Restauracja — kwiat nachodzi na stolik z parasolem

> "Pod restauracją kwiat nachodzi na stolik z parasolem i się zlewają."

**Naprawa:** przesunąć kwiat lub stolik, sprawdzić collision pre-flight (memory: żelazna reguła).

---

## 137. Twitter — sponsored post fail blokuje inne posty

> "Na twitterze próbowałem wrzucić post sponsorowany, nie mogłem bo nie miałem 500 obserwujących, ale po tym nie mogę dodać innego postu."

**Diagnoza:** prawdopodobnie `G._twitterLocked=true` ustawione przy fail, ale brak reset.

**Naprawa:** odróżnić "sponsored locked" od "regular post locked", reset stanu po fail-canceled action.

---

## 138. Wskazówki pop-up — "mądrzejsze", gdy gracz nic nie robi

> "Wyskakujące wskazówki zbyt często się powtarzają. Pop up powinny być bardziej 'mądre', powinny wyskakiwać gdy gracz nie robi żadnej akcji."

**Naprawa:**
- Idle detector (gracz nie ruszył się ~30s + brak akcji E/P/M)
- Pop-up TYLKO w idle
- Kolejka unique tipów (nie pokazywać tego samego)
- Cooldown ogólny min. 5 minut między pop-upami

---

## 139. Breakdance 30s, Skatepark 25s

> "Breakdance i skatepark trwają zbyt długo. Ustaw breakdance na 30 sekund, skatepark na 25 sekund."

**Naprawa:** sprawdzić duration w minigrach (`BREAKDANCE_DURATION`/`SKATEPARK_DURATION` lub podobne) i ustawić na 30s/25s.

---

## 140. Pizzeria — usunąć 2 kwiaty doniczki

> "Pod pizzerią są 2 kwiaty dekoracyjne w doniczkach, usuń je, bo nachodzą na futurystyczne budynki dekoracyjne."

**Naprawa:** locate i usunąć 2 emoji/sprite kwiatów przy pizzerii.

---

## 141. Apt. premium — ławka/lampa/drzewo na chodniku

> "Obok apartamentu premium stoi ławka nachodząca na chodnik, lampa na środku chodnika i pod lampą jest jakieś drzewo/krzew, też stoi na środku chodnika, ogarnij to."

**Naprawa:** przesunąć ławkę poza chodnik (na trawnik), lampę poza chodnik (przesunąć w bok), usunąć drzewo z chodnika. Audyt collision.

---

## 142. Niedobór snu — gracz dostaje pieniądze, czy ma sens

> "Gracz dostaje pieniądze za niedobór snu, zweryfikuj z czym to jest związane (może jakieś osiągnięcie) i czy to ma sens."

**Diagnoza:** sprawdzić achievement / random event przy niedoborze snu. Możliwe że error → reward path nieprawidłowo wpięty.
